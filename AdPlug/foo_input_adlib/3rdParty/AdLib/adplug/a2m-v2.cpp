...
t4OP_DATA Ca2mv2Player::get_4op_data(uint8_t chan)
{
    ...
    if (d.ins1 && d.ins2) {
        ...
        if (ins1 && ins2) {
            d.conn = (ins1->fm.connect << 1) | ins2->fm.connect;
        } else {
            d.ins1 = 0;
            d.ins2 = 0;
            d.conn = 0;
...
void Ca2mv2Player::slide_volume_up(int chan, uint8_t slide)
{
    ...
    if (!_4op_vol_valid_chan(chan)) {
        ...
        if (!ins)
            ins = get_instr_data_by_ch(chan);

        limit1 = (ins && ch->peak_lock[chan]) ? ins->fm.volC : 0;
        limit2 = (ins && ch->peak_lock[chan]) ? ins->fm.volM : 0;
    }
    ...
    case 0:
        if (!_4op_vol_valid_chan(chan)) {
            ...
            uint8_t limit1_volC = (ins1 && ch->peak_lock[d.ch1]) ? ins1->fm.volC : 0;
            uint8_t limit1_volM = (ins1 && ch->peak_lock[d.ch1]) ? ins1->fm.volM : 0;
            uint8_t limit2_volC = (ins2 && ch->peak_lock[d.ch2]) ? ins2->fm.volC : 0;
            uint8_t limit2_volM = (ins2 && ch->peak_lock[d.ch2]) ? ins2->fm.volM : 0;
