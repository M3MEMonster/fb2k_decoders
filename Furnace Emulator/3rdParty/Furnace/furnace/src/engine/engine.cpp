...
bool DivEngine::quit(bool saveConfig) {
  ...
  song.unload();
  // rePlayer begin (modified for foo_furnace)
  // Do NOT delete sysDefs here — they are static and shared across
  // all DivEngine instances.  Deleting them while another engine is
  // still alive causes use-after-free crashes.
  // rePlayer end
  return true;
}
